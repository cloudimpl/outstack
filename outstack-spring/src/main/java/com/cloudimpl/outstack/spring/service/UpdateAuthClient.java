 ...
 * @author nuwan
 */
public class UpdateAuthClient extends EntityCommandHandler<AuthClient, AuthClientCommand, AuthClient> {

    @Override
    protected AuthClient execute(EntityContext<AuthClient> context, AuthClientCommand command) {
        return context.update(command.getClientId(), new AuthClientUpdated(command.getClientId(), command.getSecret(), command.getRedirectUrls()));
    }
